odoo.define('theme_vouge.biz_quickview', function(require){
'use strict';
...
$(document).ready(function(){
    $(document).on('click', 'a.similar_btn', function () {
        var pid = $(this).attr('data-product_template_id');
        window.select_btn = $(this);
        ajax.jsonRpc('/get_similar_products', 'call', {'prod_id':pid}).then(function(data) 
        {
            $(".similar-sidebar-content").first().before(data).end().remove();
            setTimeout(function(){
                $(".similar-sidebar-content").addClass("toggled");
            }, 500);
    ...
    startLoadMore: function () {
        ...
        $('#wrapwrap').scroll(function() {
            ...
            if ($(".vouge_shop").hasClass("vouge_shop")) {
                var trigger_element = document.querySelector('.vouge_bottom_pager');
