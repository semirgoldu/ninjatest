odoo.define('theme_vouge.image_hotspot_options', function(require) {
    'use strict';
    ...
    // ajax.loadXML('/theme_vouge/static/src/xml/image_hotspot_info.xml', qweb);
    ...
    options.registry.hotspot_posi = options.Class.extend({
        xmlDependencies: ['/theme_vouge/static/src/xml/image_hotspot_info.xml'],
    	events:{
            ...
            'click we-button.show_dynamic_preview':'_showdynamicpreview',
	    ...
	    _hotspotinfotype: function () {
	    	if (this.$target.hasClass("hotspot_static")) {
                var static_info = $(qweb.render("theme_vouge.static_image_hotspot_info"));
                this.$target.find('.static_image_hotspot_info').remove();
                this.$target.find('.dynamic_image_hotspot_info').remove();
	    ...
	    _showstaticpreview: function () {
         ...
         _showdynamicpreview: function () {
            if (this.$target.find(".dynamic_image_hotspot_info").hasClass('show_edit')) {
                this.$target.find(".dynamic_image_hotspot_info").removeClass('show_edit')
            } else {
                this.$target.find(".dynamic_image_hotspot_info").addClass('show_edit')
            }
        },

        _selectproductoption: function () {
           var self = this.$target;
            self.$modal = $(qweb.render("theme_vouge.dynamic_image_hotspot_product_select"));
            ...
            ajax.jsonRpc('/theme_vouge/hotspot_product_select', 'call', {}).then(function(res) {
           ...
           $pro_sub_data.on('click', function() {
                ...
                /*$.get("/theme_vouge/get_dynamic_hotspot_product_select", {
                    ...
                    if (data) {
                        ...
                        $(data).appendTo(self);
                        ajax.jsonRpc('/theme_vouge/get_dynamic_hotspot_product_select_two', 'call', {
