odoo.define('theme_vouge.product_configurator_options', function(require) {
    'use strict';
    ...
    options.registry.theme_vouge_product_configurator = options.Class.extend({
        xmlDependencies: ['/theme_vouge/static/src/xml/product_configurator.xml'],
        events:{
        ...
        prod_config: function(type, value) {
            ...
            if (type != undefined && type.type == "click" || type == undefined) {
                ...
                self.$modal = $(qweb.render("theme_vouge.select_product_configurator"));
                ...
                /*$.get("/theme_vouge/get_product_configurator_products", {
                ...
                $('.form-product-search').keyup(function(){
                    ...
                    if (input_value.length > 2){
                        $.get("/theme_vouge/get_product_configurator_products", {
                            ...
                            if (data) {
                                self.$modal.find(".searched_products").empty()
                                self.$modal.find(".searched_products").append(data);
                ...
                /* ek var configuration save karya pachi biji var open kare to saved config select lave start */
                    ...
                    function bringSelectedProducts() {
                        ...
                        $.get("/theme_vouge/get_selected_product_configurator_products", {
                    ...
                    if (self.$target.hasClass("grid_style")){
                        ...
                        self.$modal.find('.layout_style').removeClass('d-none')
                        self.$modal.find('.display_options').removeClass('d-none')
                    }
                    if (self.$target.hasClass("list_style")){
                        ...
                        self.$modal.find('.slider_autoslide').addClass('d-none')
                        self.$modal.find('.slider_slidespeed').addClass('d-none')
                        self.$modal.find('.layout_style').addClass('d-none')
                        self.$modal.find('.display_options').addClass('d-none')
                    ...
                    if (self.$target.hasClass("show_similar_products")){
                        self.$modal.find("a[display_options=show_similar_products]").addClass('active')
                        self.$modal.find(".product_preview").addClass('show_similar_products')
                    }
                    ...
                    if (self.$target.hasClass("show_similar_products")){
                        self.$modal.find("a[display_options=show_similar_products]").addClass('active')
                        self.$modal.find(".product_preview").addClass('similar_products')
                    }
                ...
                $pro_sub_data.on('click', function() {
                    var selected_products = self.$modal.find(".selected_product > div")
